    ...
    def __call__(self, features, return_tensors=None):
        ...
        if batch.get("labels", None) is not None:
            if return_tensors == "pt":
                ...
                contains_float = any(0 < num < 1 for sublist in batch["labels"] for num in sublist)
                dtype = torch.float if contains_float else torch.int64
                batch["labels"] = torch.tensor(np.array(batch["labels"]), dtype=dtype)
            elif return_tensors == "tf":
                import tensorflow as tf
