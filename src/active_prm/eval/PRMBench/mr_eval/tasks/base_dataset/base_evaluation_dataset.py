...
from ...utils.utils import append_jsonl, gather_dict_lists, process_jsonl
    ...
    def resume_from_ckpt(self, ckpt_path):
        if os.path.exists(ckpt_path):
            ...
            for ckpt_item in ckpt_data:
                ...
                assert (
                    ckpt_item["model_name"] == self.model_name
                ), f"ckpt model name {ckpt_item['model_name']} not match with current model name {self.model_name}"
            ...
            logger.info(
                f"Total items: {len(self.full_data)}, processed items: {len(self.results)}, remaining items: {len(self.meta_data)}"
            )
        else:
...
class DataCollatorForSupervisedDataset:
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        if "model_type" in instances[0] and (
            instances[0]["model_type"] == "openai" or instances[0]["model_type"] == "vllm"
        ):
