...
from active_prm.eval.PRMBench.mr_eval.utils.log_utils import get_logger
from active_prm.eval.PRMBench.mr_eval.utils.task_utils import *
from active_prm.eval.PRMBench.mr_eval.utils.utils import *
...
def evaluate_function(results, meta_data):
    ...
    assert (
        abs(len(filtered_results) - len(meta_data)) < 5
    ), f"filtered_results number: {len(filtered_results)}, meta_data number: {len(meta_data)}"
    ...
    for prediction in correct_predictions:
        ...
        if "validity" in prediction and not prediction["validity"]:
            ...
            labels = (
                prediction["scores"]["step_level_validity_labels"]
                if "step_level_validity_labels" in prediction["scores"]
                else None
            )
    ...
    for prediction in other_predictions:
        ...
        if "validity" in prediction and not prediction["validity"]:
            ...
            if (
                classifcation == "redundency" or classifcation == "circular"
            ) and "step_level_redundancy_labels" in prediction["scores"]:
    ...
    total_final_results = {
        metric: sum(total_metric_lists[metric]) / len(total_metric_lists[metric])
        if len(total_metric_lists[metric]) > 0
        else -1
        for metric in metric_types + ["similarity"]
    }
    hallucination_type_final_results = {
        metric: {k: sum(v) / len(v) if len(v) > 0 else -1 for k, v in halucination_specified_dict[metric].items()}
        for metric in metric_types + ["similarity"]
    }
