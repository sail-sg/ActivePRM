            ...
            if big_id_meta[0]["dataset"] == dataset_name:
                ...
                for item in big_id_res:
                    if "validity" in item and not item["validity"]:
                        continue
                    step_scores = (
                        item["scores"]["step_level_validity_scores"]
                        if "step_level_validity_scores" in item["scores"]
                        else []
                    )
                ...
                completion_score_info = sorted(
                    completion_score_info, key=lambda x: x["completion_score"], reverse=True
                )
