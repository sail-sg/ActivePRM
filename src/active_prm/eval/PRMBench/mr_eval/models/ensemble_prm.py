...
from transformers import (AutoModel, AutoTokenizer, LlamaModel,
                          LlamaPreTrainedModel, MistralModel,
...
class EnPRM(prm):
    def __init__(
        ...
        self.model = AutoModelForEnsemblePRM.from_pretrained(
            pretrained,
                    ...
                    try:
                        ...
                        score_dict = dict(
                            ...
                            step_level_validity_labels=[
                                item > self.validity_threshold for item in step_level_validity_scores
                            ],
                        )
...
def make_step_rewards(logits, token_masks):
    ...
    for i in range(logits.size(1)):
        _logits = logits[:, i, token_masks[i]]
        _logits = _logits.mean(dim=0)
        all_scores_res.append(_logits.cpu().tolist())
